...
This skill helps you convert structured source material (books, syllabuses, podcast transcripts, articles) into scenario-based quiz courses for the gitquiz system, following the established format and conventions.
...
- Have structured source material (book, syllabus, transcript, article) you want to convert to an interactive quiz
...
```bash
node .opencode/skill/syllabus-to-quiz/validation.js
```
...
```bash
node .opencode/skill/syllabus-to-quiz/review_detection.js
```
...
```bash
node .opencode/skill/syllabus-to-quiz/cross_chapter_repetition.js
```
...
```bash
node .opencode/skill/syllabus-to-quiz/coverage_check.js
```
...
Run `node .opencode/skill/syllabus-to-quiz/difficulty_audit.js` (after editing `dir` inside it) to print every question organized by chapter with blank `[_]` brackets to label E/M/H.
...
Run `node .opencode/skill/syllabus-to-quiz/validate_all.js` from the gitquiz root to validate **every course** in `courses_list.txt` at once. It auto-discovers courses and runs all structural checks:
- JSON parseability
